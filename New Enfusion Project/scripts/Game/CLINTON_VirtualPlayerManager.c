class CLINTON_VirtualPlayerManager
{
	...
	void remove_bot(int id)
	{
		CLINTON_Virtual_Player botPlayer = GetPlayer(id);
		if(!botPlayer)
		{
			Print("Can not get player! 	| 	CLINTON_VirtualPlayerManager.c", LogLevel.ERROR);
			return;
		}
		IEntity character = botPlayer.GetCurrentCharacter();
		if(!character)
		{
			Print("Can not get chracter! 	| 	CLINTON_VirtualPlayerManager.c", LogLevel.ERROR);
			return;
		}
		SCR_EntityHelper.DeleteEntityAndChildren(character);
		m_aBots.RemoveOrdered(id);
	}
	...
	array<int> remove_bots_on_each_team(int quantity, array<ref SCR_BotPlayerListEntry> widgets_representing_bot)  // Retconned
	{
		...
		map<string, int> each_faction_count = new map<string,int>();
		foreach(string faction : m_aFactionsList)
		{
		...
		while( total_count > 0)
		{
			...
			if( each_faction_count.Get(bot_faction) < quantity )
			{
				...
				m_aBots.RemoveOrdered(bot_list_iterator);
	...
	void PromptRespawn(int id)
	{
		...
		SCR_EntityHelper.DeleteEntityAndChildren(character);
		botPlayer.SetCharacterAlive(false);
	}
	
	void change_bot_preference(int id, int pref)
	{
		// PrintFormat("Changing bot %1 preference %2 | remove_bot", id, pref);
		GetPlayer(id).SetLoadoutPreference(pref);
	}
	
	void change_bot_loadout(int id, int pref)
	{
		// PrintFormat("Changing bot %1 preference %2 | remove_bot", id, pref);
		CLINTON_Virtual_Player botPlayer = GetPlayer(id);
		if(!botPlayer)
		...
		botPlayer.SetLoadoutIndex(pref);
		botPlayer.SetLoadoutPreference(2); // Set loadout once
		IEntity character = botPlayer.GetCurrentCharacter();
		if(!character)
		{
			Print("Can not get chracter! 	| 	CLINTON_VirtualPlayerManager.c", LogLevel.ERROR);
...
class CLINTON_Virtual_Player
{
	...
	{
		// A bot's Id is just their position in the a_bots array
		if ( !m_bCharacterAlive)
			m_bCharacterAlive = false;
		if ( !m_bCharacterWaitingToRespawn )
		...
		if ( !m_sSurname)
			m_sSurname 	 = "";
		if ( !m_bUseCustomNames )
			m_bUseCustomNames = false;
		if ( !m_iLoadoutPreference )
	...
	bool spawnIn(map<string, ref array<SCR_AIGroup>> groups)
	{
		...
		switch(this.GetLoadoutPreference()) 
		{
			...
			case 0:
			{
				...
				int loadout_index;
				foreach( CLINTON_Virtual_Player p : players)
				{
					if (p.GetFactionKey() != this.GetFactionKey()) 	continue;
					if (this == p) 	continue;
		...
		check_deaths();
		ChimeraCharacter char; // Let's get a character name
		if(m_sCharacterName == "")
		{
			if(m_bUseCustomNames != true)
			{
				...
				{  // If no alias
					...
					preferedName = string.Format("%1 %2 %3", WidgetManager.Translate(name), WidgetManager.Translate(alias), WidgetManager.Translate(surname));
				}
		...
		SCR_CharacterIdentityComponent idComp = SCR_CharacterIdentityComponent.Cast(
					char.FindComponent(SCR_CharacterIdentityComponent));
		ref Identity ident = idComp.GetIdentity();
		...
		// Select group based on distance
		...
		// Join groups with players first
		...
		// Join a group and even out the group sizes
	...
	static bool SnapCompare(SSnapSerializerBase lhs, SSnapSerializerBase rhs, ScriptCtx ctx)
	{
		...
		return lhs.CompareStringSnapshots(rhs)  // m_String
			...
			&& lhs.CompareSnapshots(rhs, 4+4);  // m_Int 2x
	}
	...
	static bool PropCompare(CLINTON_Virtual_Player instance, SSnapSerializerBase snapshot, ScriptCtx ctx)
	{
		...
		return snapshot.CompareString(instance.m_sFactionKey)
		    ...
		    && snapshot.CompareInt(instance.m_iLoadout) // add the loadout
		    && snapshot.CompareInt(instance.m_iLoadoutPreference); // add the loadout
	}
	...
	static bool Extract(CLINTON_Virtual_Player instance, ScriptCtx ctx, SSnapSerializerBase snapshot)
	{
        ...
        snapshot.SerializeInt(instance.m_iLoadout);
        snapshot.SerializeInt(instance.m_iLoadoutPreference);
