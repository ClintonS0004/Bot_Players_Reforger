class SCR_AIFindVehicleAIAgent: AITaskScripted
{
	...
	{
		//CLINTON_RoboPlayerManager getInstance()
		m_world = owner.GetWorld(); 
		m_group = SCR_AIGroup.Cast(owner.GetParentGroup());/*  We might need to find the group or find an alternative
		if (!m_group)
		{
			SCR_AgentMustBeAIGroup(this, owner);
		}*/
		m_groupUtilityCompoment = m_group.GetGroupUtilityComponent();
		//m_groupUtilityCompoment = SCR_AIUtilityComponent.Cast(owner.FindComponent(SCR_AIUtilityComponent));
        if (!m_groupUtilityCompoment)
            Print("WARNING WARNING POO BUM",LogLevel.ERROR);
	...
	{
		SCR_ChimeraAIAgent character = SCR_ChimeraAIAgent.Cast(owner);
		
		if (!character)
			return ENodeResult.FAIL; // Could result in agents skipping logic
		/*owner;
		AIAgent k = m_mAgentsReservedCompartment.GetKey(0);
		//int i = 0
		for( int i = 0; i < m_mAgentsReservedCompartment.Count(); i = i + 1)  // Count is always zero
		{
			k = m_mAgentsReservedCompartment.GetKey(i);
			m_mAgentsReservedCompartment.Get(k);
		}
		for (AIAgent a : m_mAgentsReservedCompartment)
		{
			a;
		}*/
		
		if (m_RoboPlayerManager.QueryPlayerSeatingArangement(character))
			return ENodeResult.FAIL; // Already has a reservation 
		...
		if (m_VehicleToTestForCompartments && !HasNoAvailableCompartment(m_VehicleToTestForCompartments)) // the vehicle was found already, does it have available compartment?
		{
			...
			SetVariablesOut(character, m_VehicleToTestForCompartments, m_CompartmentType, m_Compartment);
		...
		{ 
			...
			SCR_AIBoardingWaypointParameters temp = new SCR_AIBoardingWaypointParameters();
			temp.m_bIsDriverAllowed = true;
			temp.m_bIsGunnerAllowed = true;
			temp.m_bIsCargoAllowed  = true;						
			
			if (!m_WaypointParameter) // waypoint can be deleted in the meantime
				m_WaypointParameter = temp.GetParameters();
				// return ENodeResult.FAIL;
			...
			if (m_VehicleToTestForCompartments)
			{
				if (m_bReserveCompartment)
					m_group.AllocateCompartment(m_Compartment);
					// m_mAgentsReservedCompartment.Set(owner,m_Compartment);
					m_RoboPlayerManager.SetPlayerSeat(character,m_Compartment);
